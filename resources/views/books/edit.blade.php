{{--
    ...
    This page displays the form for editing an existing book with:
    - Pre-filled form with current book data
    - Option to replace cover image
    - Option to remove existing cover image
        ...
        <div class="mb-6">
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2">
                    ...
                    <li>
                        <a href="{{ route('books.show', $book) }}" class="text-sm font-medium text-gray-500 hover:text-gray-700">
        ...
        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('books.update', $book) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div class="sm:col-span-1">
                                <label for="accession_number" class="block text-sm font-medium text-gray-700">
                                    Accession Number <span class="text-danger-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    name="accession_number"
                                    id="accession_number"
                                    value="{{ old('accession_number', $book->accession_number) }}"
                            ...
                            <div class="sm:col-span-1">
                                ...
                                >
                                    @foreach ($conditions as $value => $label)
                                        <option value="{{ $value }}" {{ old('condition', $book->condition) === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div>
                                ...
                                <div class="flex items-start space-x-4">
                                    <div class="flex-shrink-0">
                                        <img
                                            ...
                                            alt="{{ $book->title }}"
                                            class="h-32 w-24 object-cover rounded-lg shadow"
                                        >
                                    </div>
                                    <div class="flex-1">
                                        ...
                                        <form action="{{ route('books.remove-cover', $book) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to remove the cover image?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center px-3 py-1.5 border border-danger-300 shadow-sm text-sm font-medium rounded-md text-danger-700 bg-white hover:bg-danger-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-danger-500">
                        ...
                        <div>
                            <label for="cover_image" class="block text-sm font-medium text-gray-700 mb-1">
                                {{ $book->cover_image ? 'Replace Cover' : 'Upload Cover' }}
                            </label>
                ...
                <div class="px-4 py-3 bg-gray-50 text-right sm:px-6 space-x-3 rounded-b-lg">
                    <a href="{{ route('books.show', $book) }}" class="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                        Cancel
                    </a>
